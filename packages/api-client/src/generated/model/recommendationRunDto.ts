/**
 * Generated by orval v8.16.0 🍺
 * Do not edit manually.
 * logidash API
 * Logistics dispatch API — contract-first OpenAPI surface. All business routes are versioned under /v1.
 * OpenAPI spec version: 0.1.0
 */
import type { RecommendationCandidateDto } from './recommendationCandidateDto';
import type { ScoringWeightsDto } from './scoringWeightsDto';

export interface RecommendationRunDto {
  id: string;
  deliveryId: string;
  requestedByUserId: string;
  weights: ScoringWeightsDto;
  /** Eligible first (by rank asc), then ineligible */
  candidates: RecommendationCandidateDto[];
  createdAt: string;
}
